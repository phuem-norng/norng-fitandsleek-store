...
export default function AdminLayout() {
  ...
  const peopleNavGroup = useMemo(
    () => ({
      key: "users",
      label: t('navUsers') || "Users",
      icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z",
      children: [
        { path: "/admin/customers", label: t('navCustomers') || "Customers" },
        { path: "/admin/administrators", label: t('navAdministrators') || "Administrators" },
      ],
    }),
    [t]
  );
  ...
  const configNavGroup = useMemo(
    () => ({
      ...
      children: [
        { path: "/admin/settings", label: t('navGeneral') || "General" },
        { path: "/admin/homepage", label: t('navHomePage') || "Home Page" },
        { path: "/admin/homepage-complete", label: t('navCompleteManager') || "Complete Manager" },
        { path: "/admin/chatbot", label: t('navChatbot') || "Chatbot" },
        { path: "/admin/profile", label: t('navMyProfile') || "My Profile" },
      ],
    }),
    [t]
  );
    ...
    return (
      <li key={item.path} className="relative">
        <RailTooltipWrap className="w-full" labelsVisible={sidebarOpen} tip={item.label}>
          <NavLink
            to={item.path}
            end={item.path === "/admin" || item.path === "/admin/payments"}
