...
import { scanLotDiscountHint } from "../../lib/inventoryLotHelpers.js";
...
function PosUnitPriceRow({ unitPrice, comparePrice, percentOff, size = "sm" }) {
    const price = Number(unitPrice) || 0;
    const compare = Number(comparePrice);
    const showCompare = Number.isFinite(compare) && compare > price;
    const textSize = size === "base" ? "text-base" : "text-sm";

    if (!showCompare) {
        return <span className={`font-semibold text-slate-900 dark:text-white ${textSize}`}>{formatMoney(price)}</span>;
    }

    return (
        <span className={`inline-flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 ${textSize}`}>
            <span className="text-slate-400 line-through">{formatMoney(compare)}</span>
            <span className="font-semibold text-slate-900 dark:text-white">{formatMoney(price)}</span>
            {percentOff > 0 ? (
...
<div style="margin-top:12px;font-size:1rem;font-weight:800;color:#111">${escHtml(String(kh.amount_label || ""))}</div>${kh.expires_at
...
export default function AdminPosScan() {
    ...
    const canViewCheckout = can("checkout", "view");
    const canLookupCheckout = can("checkout", "view");
    const canRunCheckout = can("checkout", "create");
    ...
    const pickupMaxQty = useMemo(() => {
        const raw = pickupPreview?.max_sellable_qty;
    ...
    useEffect(() => {
        if (pickupMaxQty == null) return;
        setPickupQty((prev) => {
            const n = parseInt(String(prev || "1"), 10);
            const safe = Number.isFinite(n) ? n : 1;
            if (pickupMaxQty <= 0) return "0";
            return String(Math.min(Math.max(1, safe), pickupMaxQty));
        });
    }, [pickupMaxQty]);
    ...
    const lookupPickupCode = useCallback(async (codeArg, { silentEmpty = false } = {}) => {
        ...
        if (!canLookupCheckout) {
            setPickupPreview(null);
            setPickupErr("You don't have permission to look up products.");
            return null;
        }
    ...
    const submitPickupScanSale = useCallback(
        async () => {
            if (!canRunCheckout) {
                setPickupErr("You don't have permission to complete sales.");
                return;
            }
            ...
            if (pickupMaxQty != null && pickupMaxQty <= 0) {
                setPickupErr("This item is out of stock.");
                return;
            }
            if (pickupMaxQty != null && qty > pickupMaxQty) {
                setPickupErr(`Only ${pickupMaxQty} available in stock.`);
                return;
            }
            if (!code) {
        ...
        [pickupPreview, pickupQty, pickupMaxQty, canRunCheckout]
    );
    ...
    const appendLookupToCart = useCallback(async (rawText) => {
        ...
        if (!t) return false;
        if (!canLookupCheckout) {
            setScanErr("You don't have permission to look up products.");
            return false;
        }
        setScanErr("");
        try {
            ...
            const lotHint = scanLotDiscountHint(row);
            ...
            const maxSellable =
                row.max_sellable_qty != null && row.max_sellable_qty !== ""
                    ? Math.max(0, Number(row.max_sellable_qty))
                    : null;
            if (maxSellable != null && Number.isFinite(maxSellable)) {
                if (maxSellable <= 0) {
                    setScanErr("This item is out of stock.");
                    return false;
                }
                if (nextQty > maxSellable) {
                    setScanErr(`Only ${maxSellable} available in stock.`);
                    return false;
                }
            }
            ...
            setLines((prev) => {
                ...
                if (idx >= 0) {
                    ...
                    next[idx] = {
                        ...
                        max_sellable_qty: maxSellable ?? cur.max_sellable_qty ?? null,
                        comparePrice: lotHint?.comparePrice ?? cur.comparePrice ?? null,
                        discountPercentOff: lotHint?.percentOff ?? cur.discountPercentOff ?? null,
                    };
                    return next;
                }
                return [
                    ...prev,
                    {
                        ...
                        name,
                        unitPrice,
                        comparePrice: lotHint?.comparePrice ?? null,
                        discountPercentOff: lotHint?.percentOff ?? null,
    ...
    }, [appendLookupToCart, stopPickupCamera, stopScanCamera, canLookupCheckout]);
    ...
    const saveDraftOrder = async () => {
        if (!canRunCheckout) {
            await errorAlert({ khTitle: "គ្មានសិទ្ធិ", enTitle: "Not allowed", detail: "You don't have permission to save POS orders." });
            return;
        }
    ...
    const confirmSale = async (opts = {}) => {
        if (!canRunCheckout) {
            await errorAlert({ khTitle: "គ្មានសិទ្ធិ", enTitle: "Not allowed", detail: "You don't have permission to complete sales." });
            return;
        }
    ...
    const emailReceipt = () => {
        ...
        const body = encodeURIComponent(
            `${receiptSettings.businessName}\nTotal: ${formatMoney(lastReceipt.subtotal)}${lastReceipt.orderNumber ? `\nOrder: ${lastReceipt.orderNumber}` : ""
    ...
    if (permissionsReady && !canViewCheckout) {
                ...
                <div className="flex min-h-0 flex-1 flex-col px-4 pb-4 pt-5 sm:px-5 sm:pb-6 sm:pt-6">
                    ...
                    {PAYMENT_KHQR_UI ? (
                        <button
                            ...
                            className={`mt-2 flex w-full min-h-[3.75rem] items-center gap-3 rounded-2xl border px-4 py-3.5 text-left transition ${selectedPayment === PAYMENT_KHQR_UI.id
                        ...
                        {PAYMENT_METHODS_GRID_UI.map((m) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex min-h-[5.25rem] flex-col items-center justify-center gap-1 rounded-2xl border px-1.5 py-2.5 text-center text-xs font-semibold leading-tight transition ${active
                                ...
                                <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                                    ...
                                    <div className="w-full shrink-0 sm:w-28">
                                        ...
                                        <input
                                            ...
                                            type="number"
                                            min={pickupMaxQty != null && pickupMaxQty <= 0 ? 0 : 1}
                                            max={pickupMaxQty != null ? pickupMaxQty : 9999}
                                            value={pickupQty}
                                            onChange={(e) => {
                                                const raw = parseInt(e.target.value, 10);
                                                if (!Number.isFinite(raw)) {
                                                    setPickupQty(e.target.value);
                                                    return;
                                                }
                                                const cap = pickupMaxQty != null ? pickupMaxQty : 9999;
                                                setPickupQty(String(Math.min(Math.max(pickupMaxQty != null && pickupMaxQty <= 0 ? 0 : 1, raw), cap)));
                                            }}
                                            disabled={pickupBusy || pickupCameraOn || (pickupMaxQty != null && pickupMaxQty <= 0)}
                                    ...
                                    <div className="flex shrink-0 flex-wrap gap-2">
                                        {canRunCheckout ? (
                                            <button
                                                ...
                                                disabled={
                                                    ...
                                                    pickupLookupBusy ||
                                                    !pickupPreview ||
                                                    (pickupMaxQty != null && pickupMaxQty <= 0)
                                                }
                                        ...
                                        {pickupCameraOn ? (
                                            <button
                                                ...
                                                className={`h-12 rounded-xl border px-4 text-sm font-semibold transition ${isDark
                                ...
                                {pickupPreview ? (
                                    <div className={POS_LINE_CARD}>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            {pickupPreview.price_source === "inventory_lot" && pickupPreview.listing_status ? (
                                                <p className="mt-1 text-xs text-slate-600 dark:text-slate-300">
                                                    Inventory lot price · {String(pickupPreview.listing_status).replace(/_/g, " ")}
                                                </p>
                                            ) : null}
                                            {pickupMaxQty != null ? (
                                                <p
                                                    className={`mt-1 text-xs font-semibold ${pickupMaxQty > 0 ? "text-slate-600 dark:text-slate-300" : "text-red-600 dark:text-red-400"
                                                        }`}
                                                >
                                                    {pickupMaxQty > 0 ? `In stock: ${pickupMaxQty}` : "Out of stock"}
                                                </p>
                                            ) : null}
                                            ...
                                            <div className="mt-3 flex flex-wrap items-baseline justify-between gap-2 border-t border-slate-200/80 pt-3 dark:border-slate-600/70">
                                                <span className="text-sm text-slate-600 dark:text-slate-400">
                                                    Unit price{" "}
                                                    <PosUnitPriceRow
                                                        unitPrice={pickupPreview.price}
                                                        comparePrice={scanLotDiscountHint(pickupPreview)?.comparePrice}
                                                        percentOff={scanLotDiscountHint(pickupPreview)?.percentOff}
                                                    />
                                                </span>
                                                <span className="text-base font-bold tabular-nums text-slate-900 dark:text-white">
                                        ...
                                        <div className="mt-0.5 flex items-center justify-between gap-2">
                                            <span className="text-sm text-slate-500 dark:text-slate-400">
                                                <PosUnitPriceRow
                                                    ...
                                                    percentOff={line.discountPercentOff}
                                                />{" "}
