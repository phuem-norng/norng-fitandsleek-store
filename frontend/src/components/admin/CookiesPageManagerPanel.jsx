...
import { orderCookiesSectionEntries } from '../../lib/cookiesPageContent.js';
...
export default function CookiesPageManagerPanel({
  cookiesLocale,
  onLocaleChange,
  cookiesPage,
  canEdit,
  ...
  const orderedSections = orderCookiesSectionEntries(cookiesPage.sections, cookiesPage.section_order);

  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
      <h2 className="text-2xl font-bold mb-2 text-slate-900 dark:text-white">Cookies Page</h2>
      <p className="text-slate-500 dark:text-slate-400 mb-6">
        Manage the public Cookies Policy page: headings, body text, bullet lists, and section order. Section order is shared; text is edited per language.
      </p>

      <AdminLocaleToggle locale={cookiesLocale} onLocaleChange={onLocaleChange} />

      <div className="mb-8 grid gap-4 md:grid-cols-2">
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Page title</label>
          <input type="text" value={cookiesPage.title} onChange={(e) => onPageFieldChange('title', e.target.value)} disabled={!canEdit} className={inputClass} />
        </div>
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Last updated line</label>
          <input type="text" value={cookiesPage.last_updated} onChange={(e) => onPageFieldChange('last_updated', e.target.value)} disabled={!canEdit} className={inputClass} />
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Use arrows to reorder sections on the cookies page.</p>
        </div>
        {canEdit ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <label className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Bullet / list items (optional)</label>
                  {canEdit ? (
