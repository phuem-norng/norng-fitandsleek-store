  ...
  const fetchSuggestions = useCallback((term) => {
    ...
    api.get("/products", { params: { q: term, per_page: 10 }, signal: controller.signal })
      .then(({ data }) => {
        ...
        setHoveredIdx(items.length > 0 ? 0 : -1);
      })
      .catch((err) => {
        if (!isAbortError(err)) {
          ...
          <div className="flex items-center gap-3 px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 h-16 sm:h-[72px]">
            ...
            {query && (
              <button
                ...
                className="shrink-0 flex h-7 w-7 items-center justify-center rounded-full text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"
              >
                <X className="h-4 w-4" />
              </button>
            )}
            ...
            <button
              ...
              className="shrink-0 flex h-8 w-8 items-center justify-center rounded-full text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"
            >
            ...
            <button
              type="button"
              onClick={() => onClose?.()}
              className="shrink-0 text-base font-medium text-zinc-700 hover:text-zinc-900 pl-1"
            >
              {t("cancel")}
            </button>
          </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {recentSearches.map((term) => (
                        <div
                          ...
                          className="group relative inline-flex items-center gap-1.5 rounded-full border border-zinc-200 bg-zinc-100 px-4 py-2 text-sm text-zinc-800 hover:bg-zinc-200 cursor-pointer select-none"
                        >
                          ...
                          <button
                            ...
                            className="ml-0.5 flex h-4 w-4 items-center justify-center rounded-full text-zinc-400 hover:text-zinc-700"
                          >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {popularItems.map((item) => (
                      <button
                        ...
                        className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-100 px-4 py-2 text-sm text-zinc-800 hover:bg-zinc-200 hover:border-zinc-300"
                      >
                        {item.label}
                      </button>
                    ))}
                      ...
                      return (
                        <li key={product.id}>
                          <button
                            ...
                            className={`flex w-full items-center rounded-lg px-2 py-2 text-left text-[15px] ${isActive ? "bg-zinc-100" : "hover:bg-zinc-50"
                              }`}
                          >
                      ...
                      {products.map((p, index) => {
                        ...
                        return (
                          <button
                            ...
                            onMouseEnter={() => setHoveredIdx(index)}
                            className={`group flex flex-col text-left rounded-lg ${isActive ? "ring-2 ring-zinc-900 ring-offset-2" : ""
                              }`}
                          >
                            <div
                              className="relative w-full overflow-hidden rounded-lg bg-zinc-100"
                              style={{ aspectRatio: PRODUCT_CARD_ASPECT }}
                            >
                              <img
                            ...
                            <div className="mt-2 px-0.5">
                              <p className={`text-[13px] font-semibold text-zinc-900 leading-tight line-clamp-1 ${isActive ? "underline" : "group-hover:underline"}`}>
                                {p.name}
                              </p>
